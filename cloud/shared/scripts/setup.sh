...
if [[ ! -z ${INSTALL_NVIDIA_DOCKER+x} ]]; then 
  ...
  sudo apt-get update && sudo apt-get install -y --allow-unauthenticated --no-install-recommends linux-headers-generic dkms cuda-drivers
